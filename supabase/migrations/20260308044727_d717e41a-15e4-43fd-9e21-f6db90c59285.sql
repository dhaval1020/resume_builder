...
-- Create purchased_templates table
...
CREATE POLICY "Users can view their own purchases" ON public.purchased_templates FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own purchases" ON public.purchased_templates FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can insert their own resumes" ON public.saved_resumes FOR INSERT WITH CHECK (auth.uid() = user_id);
